import type { GetAttributeResponse } from '$lib/api/types.js';
...
export const GET = async ({ url, locals: { $t } }) => {
	...
	let query = db.selectFrom('Attribute').selectAll();
	if (search) query = query.where('Attribute.name', 'like', `%${search}%`);
	...
	if (relations?.length) {
		relations.forEach((relation) => {
			if (relation === 'values')
				query = query.select((eb) => [join.Attribute.with.AttributeValue(eb)]);
		});
	}
	...
	if (all) {
		...
		} satisfies GetAttributeResponse);
	}
...
export const POST = async ({ request, locals: { schemas } }) => {
	const { name, unitOfMeasure, values } = schemas.attribute.parse(await request.json());
	...
	const r = await db.transaction().execute(async (trx) => {
		...
		await trx
			.insertInto('AttributeValue')
			.values(values.map((v) => ({ name: v, attributeId: Number(result.insertId) })))
			.execute();
		return result;
	});
