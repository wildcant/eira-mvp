<script lang="ts">
	...
	import { productVariantsSchema, productsSchema, type ProductSchema } from '$lib/schemas/product';
	...
	const form = superForm(data.form, {
		validators: $productsSchema,
		dataType: 'json',
		onResult: ({ result }) => {
			if (result.type === 'failure' && result.data?.form?.errors?._errors) {
				openToast({
					data: {
						variant: 'destructive',
						title: 'Error',
						description: result.data.form.errors._errors.join(',')
					}
				});
			}
		}
	...
	const handleNewVariant = async (e: CustomEvent<ProductSchema>) => {
