<script lang="ts">
	...
	import type { AttributeItem, LocalAttribute } from '../types';
	import Switch from '$lib/components/ui/switch/switch.svelte';

	export let key: string;
	export let selectableAttributes: Readable<Attribute[]>;
	...
	effect([selectedAttribute, tags, optional], ([$selectedAttribute, $tags, $optional]) => {
		if ($selectedAttribute && $selectedAttribute.meta) {
			dispatch('change', {
				data: {
					key,
					attribute: $selectedAttribute.meta,
					optional: $optional,
					values: $tags.map((tag) => ({ id: +tag.id, name: tag.value }))
				} satisfies LocalAttribute
			});
		}
	});
</script>
