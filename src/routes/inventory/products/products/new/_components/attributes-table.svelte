<script lang="ts">
	...
	import type { LocalAttribute } from '../types';
	...
	const addNewAttribute = () => {
		$attributesField = [...$attributesField, { values: [], attribute: {}, key: uniqueId() } as any];
	};

	const updateAttribute = (e: CustomEvent<{ data: LocalAttribute }>) => {
		const { data } = e.detail;
	...
	const deleteAttribute = (e: CustomEvent<string>) => {
	...
	const selectableAttributes = derived(
		...
		([$attributesField]) =>
			attributes?.filter(
				(attr) => !$attributesField.map(({ attribute }) => attribute.id).includes(attr.id)
			) ?? []
	);
</script>
...
<Button type="button" variant="link" on:click={addNewAttribute}>
