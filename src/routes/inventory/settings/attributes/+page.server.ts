...
export const load = async ({ locals: { schemas, fetcher } }) => {
	...
	const initialData = await fetcher<GetAttributeResponse>(
		`${endpoint.url}?include=${endpoint.params.include.join(',')}`
	);

	const form = await superValidate(schemas.attribute);
...
export const actions = {
	create: async ({ request, fetch, locals: { schemas } }) => {
		const form = await superValidate(request, schemas.attribute);
